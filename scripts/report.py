...
from scripts.ilapfuncs import logfunc, cleaned_path
...
def generate_report(reportfolderbase, time_in_secs, time_HMS, extraction_type, image_input_path, 
    ...
    for root, dirs, files in sorted(os.walk(reportfolderbase)):
        ...
        for file in files:
            ...
            if file.endswith(".temphtml"):
                fullpath = Path(root).joinpath(file)
                filename = fullpath.stem
                SectionHeader = fullpath.parts[-2]
                if SectionHeader == '_elements':
                    ...
                    nav_list_data += list_item.format('', Path(filename.replace(" ", "_")).with_suffix('.html'), 
    ...
    for category, path_list in side_list.items():
        for path in path_list:
            old_filename = path.with_suffix('.html').name
            ...
            f = open(reportfolderbase.joinpath('_HTML', filename), 'w', encoding='utf8')
...
def create_index_html(reportfolderbase, time_in_secs, time_HMS, extraction_type, image_input_path, 
    ...
    case_list += [
        ['Extraction location', cleaned_path(image_input_path)],
        ['Extraction type', extraction_type],
        ['Report directory', cleaned_path(reportfolderbase)],
    ...
    devinfo_files_path = reportfolderbase.joinpath('Script Logs', 'DeviceInfo.html')
    tab2_content = get_file_content(devinfo_files_path)

    # Get script run log (this will be tab3)
    script_log_path = reportfolderbase.joinpath('Script Logs', 'Screen Output.html')
    tab3_content = get_file_content(script_log_path)

    # Get processed files list (this will be tab3)
    processed_files_path = reportfolderbase.joinpath('Script Logs', 'ProcessedFilesLog.html')
    ...
    body_description = 'iLEAPP is an open source project that aims to parse every known iOS artifact \
        for the purpose of forensic analysis.'
    active_nav_list_data = mark_item_active(nav_list_data, filename) + nav_bar_script

    html_reportfolderbase = reportfolderbase.joinpath('_HTML')
    ...
    redirection = \
        ...
        """
    f = open(reportfolderbase.joinpath(filename), 'w', encoding='utf8')
    f.write(redirection)
    f.close()
